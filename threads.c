...
void	*	learning_thread(void * args){
	...
	for (m = 0; m < NBMAPS; m++){
		// printf("****************\nBefore learning\n");
		arg->qlt_train->distortion[m][0] = distortion_measure(arg->map[m], arg->train_set[0], NBITEREPOCH, SIGMA_GAUSS);
		arg->qlt_train->distortion_gss[m][0] = distortion_measure_GSS(arg->map[m], arg->train_set[0], NBITEREPOCH, SIGMA_GAUSS); 
		
		arg->qlt_train->quantization[m][0] = avg_quant_error(arg->map[m], arg->train_set[0], NBITEREPOCH);
		arg->qlt_train->quantization_gss[m][0] = avg_quant_error_GSS(arg->map[m], arg->train_set[0], NBITEREPOCH);
		
		arg->qlt_valid->distortion[m][0] = distortion_measure(arg->map[m], arg->valid_set, NBITEREPOCH, SIGMA_GAUSS);
		arg->qlt_valid->distortion_gss[m][0] =  distortion_measure_GSS(arg->map[m], arg->valid_set, NBITEREPOCH, SIGMA_GAUSS); 
		...
		for (j = 0; j < NBEPOCHLEARN; j++) {
			...
			arg->qlt_train->distortion[m][j] = distortion_measure(arg->map[m], arg->train_set[j], NBITEREPOCH,
																  SIGMA_GAUSS);
			arg->qlt_train->distortion_gss[m][j] = distortion_measure_GSS(arg->map[m], arg->train_set[j], NBITEREPOCH,
																		  SIGMA_GAUSS);

			arg->qlt_train->quantization[m][j] = avg_quant_error(arg->map[m], arg->train_set[j], NBITEREPOCH);
			arg->qlt_train->quantization_gss[m][j] = avg_quant_error_GSS(arg->map[m], arg->train_set[j], NBITEREPOCH);

			arg->qlt_valid->distortion[m][j] = distortion_measure(arg->map[m], arg->valid_set, NBITEREPOCH,
																  SIGMA_GAUSS);
			arg->qlt_valid->distortion_gss[m][j] = distortion_measure_GSS(arg->map[m], arg->valid_set, NBITEREPOCH,
																		  SIGMA_GAUSS);

			arg->qlt_valid->quantization[m][j] = avg_quant_error(arg->map[m], arg->valid_set, NBITEREPOCH);
			arg->qlt_valid->quantization_gss[m][j] = avg_quant_error_GSS(arg->map[m], arg->valid_set, NBITEREPOCH);
		}
	}
	free(arg->status);
    ...
    for (e = 0; e < nb_experiments; e++) {
      for (m = 0; m < NBMAPS; m++) {
				...
				arg->qlt_test->quantization_gss[p][e][m] = avg_quant_error_GSS(arg->map[m], arg->test_set, NBITEREPOCH);
				arg->qlt_test->quantization_faulty[p][e][m] = avg_quant_error(map2, arg->test_set, NBITEREPOCH);
				arg->qlt_test->quantization_gss_faulty[p][e][m] = avg_quant_error_GSS(map2, arg->test_set, NBITEREPOCH);

				arg->qlt_test->distortion[p][e][m] = distortion_measure(arg->map[m], arg->test_set, NBITEREPOCH, SIGMA_GAUSS);
				arg->qlt_test->distortion_gss[p][e][m] = distortion_measure_GSS(arg->map[m], arg->test_set, NBITEREPOCH, SIGMA_GAUSS); 
				arg->qlt_test->distortion_faulty[p][e][m] = distortion_measure(map2, arg->test_set, NBITEREPOCH, SIGMA_GAUSS);
				arg->qlt_test->distortion_gss_faulty[p][e][m] = distortion_measure_GSS(map2, arg->test_set, NBITEREPOCH, SIGMA_GAUSS); 
    ...
    for (e = 0; e < nb_experiments; e++) {
      for (m = 0; m < NBMAPS; m++) {
      	arg->stat->stddev[p] += (arg->qlt_test->quantization[p][e][m] - arg->stat->avg[p]) * (arg->qlt_test->quantization[p][e][m] - arg->stat->avg[p]);
				arg->stat->stddev_gss[p] += (arg->qlt_test->quantization_gss[p][e][m] - arg->stat->avg_gss[p]) * (arg->qlt_test->quantization_gss[p][e][m] - arg->stat->avg_gss[p]);
				arg->stat->stddev_faulty[p] += (arg->qlt_test->quantization_faulty[p][e][m] - arg->stat->avg_faulty[p]) * (arg->qlt_test->quantization_faulty[p][e][m] - arg->stat->avg_faulty[p]);
				arg->stat->stddev_gss_faulty[p] += (arg->qlt_test->quantization_gss_faulty[p][e][m] - arg->stat->avg_gss_faulty[p]) * (arg->qlt_test->quantization_gss_faulty[p][e][m] - arg->stat->avg_gss_faulty[p]);

				arg->stat->stddevdist[p] += (arg->qlt_test->distortion[p][e][m] - arg->stat->avgdist[p]) * (arg->qlt_test->distortion[p][e][m] - arg->stat->avgdist[p]);
				arg->stat->stddevdist_gss[p] += (arg->qlt_test->distortion_gss[p][e][m] - arg->stat->avgdist_gss[p]) * (arg->qlt_test->distortion_gss[p][e][m] - arg->stat->avgdist_gss[p]);
				arg->stat->stddevdist_faulty[p] += (arg->qlt_test->distortion_faulty[p][e][m] - arg->stat->avgdist_faulty[p]) * (arg->qlt_test->distortion_faulty[p][e][m] - arg->stat->avgdist_faulty[p]);
				arg->stat->stddevdist_gss_faulty[p] += (arg->qlt_test->distortion_gss_faulty[p][e][m] - arg->stat->avgdist_gss_faulty[p]) * (arg->qlt_test->distortion_gss_faulty[p][e][m] - arg->stat->avgdist_gss_faulty[p]);
			}
		}
  ...
  temp = "test_evaluting_";
	...
	strcat(buffer, file_ext);
	fp = fopen(buffer, "w+");
	fprintf(fp, "PercentageFaults;ExpNumber;MapNumber;quantization;quantization_gss;quantization_faulty;quantization_gss_faulty;\
		distortion;distortion_gss;distortion_faulty;distortion_gss_faulty\n");
	for (p = 0; p < MAXFAULTPERCENT; p++) {
		for (e = 0; e < nb_experiments; e++) {
			for (m = 0; m < NBMAPS; m++) {
  ...
  temp = "statistics_";
	...
	strcat(buffer, file_ext);
	fp = fopen(buffer, "w+");
	fprintf(fp, "PercentageFaults;avg;avg_gss;avg_faulty;avg_gss_faulty;avgdist;avgdist_gss;avgdist_faulty;avgdist_gss_faulty;\
		stddev;stddev_gss;stddev_faulty;stddev_gss;stddev_gss_faulty;stddevdist;stddevdist_gss;stddevdist_faulty;stddevdist_gss_faulty");
	for (p = 0; p < MAXFAULTPERCENT; p++) {
